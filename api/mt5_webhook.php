...
foreach ($transactions as $tx) {
    if (!isset($tx['ticket']) || !isset($tx['timestamp']) || !isset($tx['type']) || 
        ...
        log_error("Datos incompletos para ticket " . ($tx['ticket'] ?? 'N/A'));
        continue;
    }
    ...
    $risk_r        = isset($tx['risk_r']) ? (float)$tx['risk_r'] : 0.0;
...
echo json_encode(["status" => "success", "transactions_processed" => $inserted]);
?>